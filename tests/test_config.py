...
class TestReservedEnv:
    ...
    @pytest.mark.parametrize(
        "key", ["DISPATCH_LOG_LEVEL", "DISPATCH_VERBOSE", "DISPATCH_LLM_INSTRUMENT"]
    )
    def test_former_overridable_toggles_now_rejected(self, key: str):
        """Behavioral toggles moved to dedicated config fields; env overrides are gone."""
        with pytest.raises(ValueError) as exc_info:
            DispatchConfig(env={key: "x"})
...
class TestUnknownKeys:
    """DispatchConfig is the source of truth and forbids unknown keys."""

    def test_unknown_key_rejected(self):
        with pytest.raises(ValueError) as exc_info:
            DispatchConfig.model_validate({"not_a_field": True})
        assert "not_a_field" in str(exc_info.value)

    def test_known_fields_accepted(self):
        config = DispatchConfig.model_validate(
            {"llm_instrument": False, "log_level": "debug"}
        )
...
class TestLogLevel:
    ...
    def test_normalized_uppercase(self, value: str, expected: str):
        assert DispatchConfig(log_level=value).log_level == expected

    def test_default_is_none(self):
        assert DispatchConfig().log_level is None

    def test_invalid_value_rejected(self):
        with pytest.raises(ValueError, match="Invalid log_level"):
            DispatchConfig(log_level="TRACE")

    def test_serialized_only_when_set(self):
        assert "log_level" not in DispatchConfig().to_yaml_dict()
        assert DispatchConfig(log_level="debug").to_yaml_dict()["log_level"] == "DEBUG"
