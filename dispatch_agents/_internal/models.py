...
class FunctionTrigger(StrictBaseModel):
    ...
    Defines how an agent function is invoked:
    - topic: Event-driven triggers from subscribed topics.
    - callable: Direct function calls from other agents via invoke().
    """

    type: Literal["topic", "callable"] = Field(
        description="Trigger type: 'topic' for event-driven or 'callable' for direct invocation"
    )
...
class PublishResponse(StrictBaseModel):
    ...
    trace_id: str | None = None  # Trace ID for the event, when available
