...
# Prefixed vars the SDK reads but that agent authors may legitimately override in
# their own ``env:`` block (behavioral toggles, not platform plumbing).
USER_OVERRIDABLE_ENV: frozenset[str] = frozenset(
    {
        "DISPATCH_LOG_LEVEL",
        "DISPATCH_VERBOSE",
    }
...
def reserved_env_keys(value: dict[str, str] | None) -> list[str]:
    ...
    A key is reserved when it starts with ``DISPATCH_`` and is not one of the
    explicitly user-overridable toggles.
    """
    if not value:
        return []
    return sorted(
        key
        for key in value
        if key.startswith(RESERVED_ENV_PREFIX) and key not in USER_OVERRIDABLE_ENV
    )
