...
from dispatch_agents.models import BasePayload, JsonObject
...
class DeploymentReviewBase(GitHubEventPayload):
    ...
    # workflow_run and reviewers use JsonObject rather than typed models because
    ...
    workflow_run: JsonObject | None = Field(
    ...
    workflow_job_run: JsonObject | None = Field(
        default=None,
        description="Workflow job run associated with the deployment review",
    )
    reviewers: list[JsonObject] | None = Field(
        default=None,
...
class GitHubStatusCommit(GitHubModel):
    ...
    commit: JsonObject = Field(description="Commit data including message and author")
...
class CommitStatus(GitHubEventPayload):
    ...
    branches: list[JsonObject] = Field(
...
class GitHubDependabotAdvisory(GitHubModel):
    ...
    cvss: JsonObject = Field(description="CVSS score and vector string details")
    cwes: list[JsonObject] = Field(description="CWE weakness identifiers")
    identifiers: list[JsonObject] = Field(
        description="External identifiers for the advisory"
    )
    references: list[JsonObject] = Field(
...
class GitHubDependabotVulnerability(GitHubModel):
    ...
    first_patched_version: JsonObject | None = Field(
        default=None,
...
class Ping(GitHubEventPayload):
    ...
    hook: JsonObject = Field(description="Webhook configuration object")
...
class PageBuild(GitHubEventPayload):
    ...
    build: JsonObject = Field(description="Page build details")
...
class TeamAdd(GitHubEventPayload):
    ...
    team: JsonObject = Field(description="Team that was given access")
...
class BranchProtectionRuleBase(GitHubEventPayload):
    ...
    rule: JsonObject = Field(description="Branch protection rule data")
...
class CommitCommentBase(GitHubEventPayload):
    ...
    comment: JsonObject = Field(description="Comment on the commit")
...
class DeployKeyBase(GitHubEventPayload):
    ...
    key: JsonObject = Field(description="Deploy key data")
...
class MembershipBase(GitHubEventPayload):
    ...
    team: JsonObject = Field(description="Team the user was added to or removed from")
...
class MergeGroupBase(GitHubEventPayload):
    ...
    merge_group: JsonObject = Field(description="Merge group data")
...
class MetaBase(GitHubEventPayload):
    ...
    hook: JsonObject = Field(description="Modified webhook data")
...
class RepositoryDispatch(GitHubEventPayload):
    ...
    client_payload: JsonObject = Field(
...
class GitHubDiscussion(GitHubModel):
    ...
    state: str = Field(description="State: open or closed")
    category: JsonObject = Field(description="Discussion category")
    author_association: str = Field(
...
class DiscussionAnswered(DiscussionBase):
    ...
    answer: JsonObject = Field(description="Comment that was marked as the answer")
...
class DiscussionUnanswered(DiscussionBase):
    ...
    old_answer: JsonObject = Field(description="Comment that was previously the answer")
...
class DiscussionCommentBase(GitHubEventPayload):
    ...
    action: str = Field(description="Event action: created, deleted, or edited")
    comment: JsonObject = Field(description="Comment on the discussion")
    discussion: GitHubDiscussion = Field(
        description="Discussion the comment belongs to"
    )
    installation: JsonObject = Field(
...
class GitHubCodeScanningAlert(GitHubModel):
    ...
    rule: JsonObject = Field(description="Rule that triggered the alert")
    tool: JsonObject = Field(description="Tool that generated the alert")
    most_recent_instance: JsonObject = Field(
...
class SecretScanningAlertLocationBase(GitHubEventPayload):
    ...
    location: JsonObject = Field(description="Location data for the secret")
...
class RepositoryVulnerabilityAlertBase(GitHubEventPayload):
    ...
    alert: JsonObject = Field(description="Vulnerability alert data")
...
class SecurityAdvisoryBase(GitHubEventPayload):
    ...
    security_advisory: JsonObject = Field(
...
class MarketplacePurchaseBase(GitHubEventPayload):
    ...
    marketplace_purchase: JsonObject = Field(
        description="Marketplace purchase details including plan and billing cycle"
    )
    previous_marketplace_purchase: JsonObject | None = Field(
        default=None,
...
class PackageBase(GitHubEventPayload):
    ...
    package: JsonObject = Field(description="Package data")
...
class RegistryPackageBase(GitHubEventPayload):
    ...
    action: str = Field(description="Event action: published or updated")
    registry_package: JsonObject = Field(description="Registry package data")
...
class ProjectBase(GitHubEventPayload):
    ...
    project: JsonObject = Field(description="Project board data")
...
class ProjectCardBase(GitHubEventPayload):
    ...
    project_card: JsonObject = Field(description="Project card data")
...
class ProjectCardEdited(ProjectCardBase):
    ...
    changes: JsonObject = Field(description="Changes made to the project card")
...
class ProjectColumnBase(GitHubEventPayload):
    ...
    project_column: JsonObject = Field(description="Project column data")
...
class ProjectColumnEdited(ProjectColumnBase):
    ...
    changes: JsonObject = Field(description="Changes made to the project column")
...
class ProjectsV2ItemBase(GitHubEventPayload):
    ...
    projects_v2_item: JsonObject = Field(description="Projects V2 item data")
...
class ProjectsV2ItemArchived(ProjectsV2ItemBase):
    ...
    changes: JsonObject = Field(description="Changes made to the project item")
...
class SponsorshipBase(GitHubEventPayload):
    ...
    sponsorship: JsonObject = Field(description="Sponsorship details")
...
class CustomPropertyBase(GitHubEventPayload):
    ...
    action: str = Field(description="Event action: created or deleted")
    definition: JsonObject = Field(description="Custom property definition data")
...
class CustomPropertyValuesBase(GitHubEventPayload):
    ...
    new_property_values: list[JsonObject] = Field(
        description="New custom property values after the update"
    )
    old_property_values: list[JsonObject] = Field(
...
class DeploymentProtectionRuleBase(GitHubEventPayload):
    ...
    pull_requests: list[JsonObject] = Field(
...
class InstallationTargetBase(GitHubEventPayload):
    ...
    account: JsonObject = Field(description="Account (user or org) that was renamed")
    changes: JsonObject = Field(description="Changes made (e.g., old login value)")
